...
function preload()
{
  ...
  bk_song = loadSound('sounds/maxkomusic-meditative-space.mp3');
...
story=createSprite(600,600,100,100);
story.addImage(storyImg);
story.scale=1;
story.visible=true;

control=createSprite(600,800,100,100);
control.addImage(controlImg);
control.visible=true;

back=createSprite(600,900,100,100);
...
function draw() 
{
 ...
 // text("Thank You For Playing this Game",)
  ...
  story.visible=false;
  fill("red");
  text("STORY: one day there were two rabbits in the forest who came to.",30,100);
  fill("orange");
  text(" the city they went into a warehouse, they saw two carrots they",30,200);
  fill("yellow");
  text(" went to catch it but they accidently stepped into a button. Suddenly ",10,300);
  fill("green");
  text("two robotic arms came and put them in the rocket the rocket blasted ",10,400);
  fill("blue")
  text("off now the rabbits are in space now they have to collect the stars ",30,500);
  fill("indigo")
  text("and the two rabbits should get the carrots then only they can go",30,600);
  fill("violet")
  text(" back to earth",30,700);
  fill("red")
  text("Enjoy The Game",400,800);
  ...
  control.visible=false;
  fill("red")
   text("Click The Play Button and Do Not Press The Thruster Image Press",10,100);
   fill("orange")
   text("The Green Button or your Friend bunny Can Press It And Now Play",10,200);
   fill("yellow")
   text("The Game Before The Oxygen Ran Out And Collect All Stars Feed ",10,300);
   fill("green")
   text("The Bunny or his Friend",10,400);
   fill("blue")
   text("Enjoy The Game",400,500);
   back.visible=true;
   if(mousePressedOver(back))
   {
     //gameState=2;
